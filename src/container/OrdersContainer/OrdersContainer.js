    ...
    axios({
      ...
      .then((response) => {
        ...
        newOrdersArr.forEach((item, index) => {
          ...
          line_items.push(
            newOrdersArr.filter((order) => order.order_id === index + 1)
          );
        });
        console.log("line items");

        line_items = line_items.reverse();
        line_items = line_items.slice(633, 643);
    ...
    const orders = this.state.fullOrders.map((order, index) => {
      return (
        <tr key={index}>
          ...
          <td>{index + 1}</td>
          <td>
            <strong>{order.slice(0, 50)}...</strong>
          </td>
          ...
          <td>
            <Button
              ...
              onClick={() =>
                this.orderDetail(
                  index + 1,
                  this.state.orderTotals[index],
                  this.state.orderDates[index]
                )
              }
            >
              View Order
      ...
      <div className="container">
        ...
        <Col md={10}>
          <Table striped bordered condensed hover>
            ...
            <tbody style={{ padding: 5 }}>{orders}</tbody>
          </Table>
          {this.state.showModal ? (
            <div className="static-modal">
              <Modal.Dialog style={{ overflowY: "initial" }}>
                <Modal.Header>
                  <Modal.Title>
                    Order Details for Order ID: {this.state.currentOrderID}
                  </Modal.Title>
                </Modal.Header>
                <Modal.Body style={{ textAlign: "left" }}>
                  ...
                  <strong>
                    Order completed on {this.state.currentOrderDate}
                  </strong>
                </Modal.Body>
                <Modal.Footer>
